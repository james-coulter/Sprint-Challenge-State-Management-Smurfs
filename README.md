...
*Context API helps mitigate prop drilling through child components. It also allows for a cleaner state management, especially in smaller applications where Redux might be too robust for.*
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
*Actions are the payloads that send all of the information to the store.Reducers defines how the state changes/evolves in response to the actions.*
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
*Application State is saved globally, where Componenet State is saved locally within the component itself. Application State is good when multiple components need the state.*
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
*Redux-thunk is a middleware that allows us to make asynchronous state calls*
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
*I really enjoy Context API, especially with these smaller applications. For some reason it 'clicked' the quickest.*
